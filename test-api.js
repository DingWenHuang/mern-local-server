...
async function registerAndLoginUser(userObj) {
  try {
    ...
    assert.equal(registerRes.body.savedUser.username, userObj.username);
    assert.equal(registerRes.body.savedUser.email, userObj.email);
    assert.equal(registerRes.body.savedUser.role, userObj.role);
...
describe("User register", () => {
  it("register - success", async () => {
    try {
      ...
      assert.equal(registerRes.status, 201);
      assert.equal(registerRes.body.msg, "成功註冊使用者");
      assert.equal(registerRes.body.savedUser.username, username);
      assert.equal(registerRes.body.savedUser.email, email);
      assert.equal(registerRes.body.savedUser.role, role);
    } catch (err) {
      console.error(err);
      throw err;
    }
